...
class _SignupOwnerScreenState extends State<SignupOwnerScreen> {
    ...
    if (_formKey.currentState!.validate() &&
      ...
      try {
        ...
        if (userCredential.user != null) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
                content: Text('Creating account and uploading images...')),
          );

          // Upload images to Firebase Storage
          validIdUrl =
              await _uploadImageToStorage(_validId!, 'validIds', userId);
          proofOfOwnershipUrl = await _uploadImageToStorage(
              _proofOfOwnership!, 'proofsOfOwnership', userId);

          if (validIdUrl != null && proofOfOwnershipUrl != null) {
            ...
            print(
                'Owner account created for $userId with images: $validIdUrl, $proofOfOwnershipUrl');
            if (mounted) {
  ...
  Future<String?> _uploadImageToStorage(
      File file, String folderName, String userId) async {
    try {
      final storageRef = FirebaseStorage.instance
          .ref()
          .child('$folderName/$userId/${Path.basename(file.path)}');
      await storageRef.putFile(file);
      return await storageRef.getDownloadURL();
    } catch (e) {
      print('Error uploading image to $folderName: $e');
      // Optionally, show a specific error message to the user from here if needed,
      // but _submitForm will also catch and show a generic one.
      // ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to upload $folderName image: $e'), backgroundColor: Colors.red));
      return null;
    }
  }
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              _buildUploadButton(
                ...
                text:
                    _validId == null ? 'Upload Valid ID' : 'Valid ID Uploaded',
              ),
              if (_validId != null) ...[
      ...
      children: [
        TextFormField(
          ...
          decoration: InputDecoration(
            ...
            labelStyle:
                const TextStyle(color: Colors.deepPurple), // Label text color
            floatingLabelBehavior: FloatingLabelBehavior.always,
